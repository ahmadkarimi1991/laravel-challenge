...
function displayCurrentTab(current) {
  for (var _i = 0; _i < tabsBoxes.length; _i++) {
    if (current === _i) {
      tabsBoxes[_i].style.display = "block";
      tabsButtons.forEach(function (element) {
        element.classList.remove("challenge-projects__tab--active");
      });

      tabsButtons[_i].classList.add("challenge-projects__tab--active");
    } else {
      tabsBoxes[_i].style.display = "none";
    }
  }
}
...
tabs.addEventListener("click", function (event) {
  if (event.target.className === "challenge-projects__tab") {
    for (var _i2 = 0; _i2 < tabsButtons.length; _i2++) {
      if (event.target === tabsButtons[_i2]) {
        displayCurrentTab(_i2);
        break;
      }
    }
  }
